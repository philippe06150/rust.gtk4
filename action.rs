...
fn main() -> gtk::glib::ExitCode {
    ...
    app.connect_startup(|app| {
		...
		let rouge_item = gio::MenuItem::new(
			Some("Rouge"),
			Some("app.rouge"),
		);
		//
		let bleu_item = gio::MenuItem::new(
			Some("Bleu"),
			Some("app.bleu"),
		);
		//
...
fn build_ui(app: &gtk::Application) {
    ...
    header_bar.set_decoration_layout(
		Some("")
	); // defaut : "menu:minimize,maximize,close"
    //
    ...
    let rouge = gio::ActionEntry::builder("rouge")
        .activate(glib::clone!(#[weak] courbes, #[weak] but1, #[strong] rvb, move |_, _, _| {
	...
	bouge.connect_drag_update( glib::clone!(
		#[strong] xy, #[weak] courbes, move |_, xx, yy| {
	...
	//
    but1.connect_clicked(glib::clone!(#[weak] window, move |_|
		window.close()
	));
	//
	// drawarea
